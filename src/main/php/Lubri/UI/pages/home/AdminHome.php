...
class AdminHome extends LubriPage{
	...
	public function parseStatsVentas(XTemplate $xtpl){
		...
		$xtpl->assign("monto_ventas", LubriUIUtils::formatMontoToView($totalesDia["monto"]) );
		...
		$xtpl->assign("monto_ventas_mes", LubriUIUtils::formatMontoToView($totalesMes["monto"]) );
	...
	public function parseDebitosCreditos( XTemplate $xtpl){
		...
		foreach ($debitosCreditos as $informeDebitoCredito) {
			...
			$xtpl->assign("titulo", LubriUIUtils::formatDateToView( $informeDebitoCredito->getFechaVencimiento()) );

			if($informeDebitoCredito->getDebito()){
				$xtpl->assign("subtitulo", LubriUIUtils::formatMontoToView($informeDebitoCredito->getDebito()) );
				$xtpl->assign("descripcion", "Débito" );
			}else{
				$xtpl->assign("subtitulo", LubriUIUtils::formatMontoToView($informeDebitoCredito->getCredito()) );
	...
	public function parseSaldos(XTemplate $xtpl){
		...
		/*}else{
			$xtpl->assign("saldo_caja", LubriUIUtils::formatMontoToView( 0) );
		}*/

		/*$xtpl->assign("saldo_cajaChica", LubriUIUtils::formatMontoToView( $this->getCajaChica()->getSaldo() ) );
		$xtpl->assign("linkMovimientosCajaChica", $this->getLinkMovimientosCajaChica());

		$xtpl->assign("saldo_bancos", LubriUIUtils::formatMontoToView( UIServiceFactory::getUIBancoService()->getSaldoBancos() ) );
		$xtpl->assign("linkMovimientosBanco", $this->getLinkMovimientosBanco());

		$xtpl->assign("saldo_ctasctes", LubriUIUtils::formatMontoToView( UIServiceFactory::getUICuentaCorrienteService()->getSaldoCtasCtes() ) );
	*/
	}
	...
	public function parseGastos( XTemplate $xtpl){
		...
		foreach ($gastos as $gasto) {
			$xtpl->assign("titulo", LubriUIUtils::formatDateToView( $gasto->getFechaVencimiento()) );
			$xtpl->assign("subtitulo", LubriUIUtils::formatMontoToView($gasto->getMonto()) );
