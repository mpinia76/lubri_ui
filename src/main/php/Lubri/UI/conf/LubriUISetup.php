...
class LubriUISetup {
	 ...
	 */
	public static function initialize( $appName = "lubri_ui"){
...
//		$_SESSION['LAST_ACTIVITY'] = time();
	...
	private static function initializeCore(){
		...
		LubriConfig::getInstance()->setDbName("cose_lubri");

		LubriConfig::getInstance()->initialize();
		LubriConfig::getInstance()->initLogger(dirname(__DIR__) . "/conf/log4php.xml");
	...
	private static function initializeUI( $appName ){
		...
		Logger::log("Logger lubri ui configurado!");
		...
		$rastyLoader->loadXml(
				...
				RastyConfig::getInstance()->getAppPath() . "src/main/php/Lubri/UI/",
		...
		//SorteosUIConfig::getInstance()->initialize("LubriMetroLayout");
