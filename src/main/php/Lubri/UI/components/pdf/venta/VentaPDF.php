...
namespace Lubri\UI\components\pdf\venta;
...
class VentaPDF extends RastyComponent{
	...
	protected function parseXTemplate(XTemplate $xtpl){
		...
		if( !empty($venta )){
			...
			$xtpl->assign( "fecha", LubriUIUtils::formatDateTimeToView($venta->getFecha()) );
			$observaciones = ($venta->getObservaciones())?' - '.$venta->getObservaciones():'';
			$xtpl->assign( "cliente", $venta->getCliente().$observaciones);
            $vehiculo = ($venta->getKm())?' - Vehículo: '.$venta->getMarca().' ('.$venta->getPatente().') KMs: '.$venta->getKm().' Próximo: '.$venta->getProxima():'';
            $xtpl->assign( "vehiculo", $vehiculo);
            $xtpl->assign( "cliente", $venta->getCliente().$observaciones);
			...
			foreach ($venta->getDetalles() as $detalle) {
				if (!empty($detalle->getCombo())) {
					...
					if(!in_array($detalle->getCombo()->getOid(), $combos)){
						...
						$xtpl->assign( "precio", LubriUIUtils::formatMontoToView( $detalle->getCombo()->getPrecio()*$cantidad ) );
						$xtpl->assign( "subtotal", LubriUIUtils::formatMontoToView( $detalle->getCombo()->getPrecio()*$cantidad  ) );
					...
					$xtpl->assign( "precio", LubriUIUtils::formatMontoToView( $detalle->getPrecioUnitario() ) );
					$xtpl->assign( "subtotal", LubriUIUtils::formatMontoToView( $detalle->getSubtotal() ) );
					$xtpl->parse( "main.detalle" );
			...
			$xtpl->assign( "total", LubriUIUtils::formatMontoToView( $total ) );
			...
			foreach ($venta->getDevoluciones() as $devolucion) {
				...
				$htmlItem .='<tr><td>'.$devolucion->getProducto().'</td><td style="text-align: right;">'.$devolucion->getCantidad().'</td><td style="text-align: right;">'.LubriUIUtils::formatMontoToView( $devolucion->getPrecioUnitario() ).'</td><td style="text-align: right;">'.LubriUIUtils::formatMontoToView( $devolucion->getSubtotal() ).'</td></tr>';
	...
	<table cellspacing="0" cellpadding="0" border="1">
		...
		<tr><td colspan="4"></td></tr>
		<tr><td colspan="4"></td></tr>
			<tr><td style="text-align: right; font-weight: bold;" colspan="3">'.$this->localize( "venta.detalle.subtotal" ).'</td><td style="text-align: right;font-weight: bold;">'.LubriUIUtils::formatMontoToView( $venta->getMontoDevolucion() ).'</td></tr>
			...
			if ($venta->getMontoActualizado()) {
				$xtpl->assign( 'htmlMontoActualizado','<tr><td style="text-align: right; font-weight: bold;" colspan="3" width="616px">'.$this->localize('venta.montoActualizado').'</td><td style="text-align: right;font-weight: bold;" width="100px">'.LubriUIUtils::formatMontoToView($venta->getMontoActualizado()),'</td></tr>');
			}
			$xtpl->assign( "totales", LubriUIUtils::formatMontoToView( $venta->getMonto() ) );
			$xtpl->assign( "pagado", LubriUIUtils::formatMontoToView( $venta->getMontoPagado() ) );
			$xtpl->assign( "adeudado", LubriUIUtils::formatMontoToView( $venta->getMontoDebe() ) );
