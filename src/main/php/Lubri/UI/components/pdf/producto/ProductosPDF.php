...
namespace Lubri\UI\components\pdf\producto;
...
use Lubri\UI\components\filter\model\UITipoProductoCriteria;
...
class ProductosPDF extends RastyComponent{
	...
	protected function parseXTemplate(XTemplate $xtpl){
		...
		$xtpl->assign( "fecha", LubriUIUtils::formatDateTimeToView(new Datetime()) );
		...
		foreach ($tipos as $tipo) {
			...
			foreach ($productos as $producto) {
				...
				$xtpl->assign( "precio", LubriUIUtils::formatMontoToView( $producto->getPrecioEfectivo() ) );
			...
			foreach ($arrTipos as $arrTipo) {
				...
				foreach ($packs as $pack) {
					if ($arrTipo->getOid()==$pack->getProducto()->getTipoProducto()->getOid()) {
						$html.='<tr><td>'.$pack->getProducto()->getMarcaProducto().' '.$pack->getProducto()->getNombre().' '.$pack->getNombre().'</td><td style="text-align: right;">'.$pack->getCantidad().'</td><td style="text-align: right;">'.LubriUIUtils::formatMontoToView($pack->getPrecioEfectivo()).'</td></tr>';
					}
			...
			if ($mostrarCombo) {
				...
				<hr />
								...
								foreach ($arrayCombos as $combo) {
									$productos='(';
									foreach ($combo->getProductos() as $value) {
										$productos.= $value->getCantidad(). ' '.$value->getProducto()->getNombre().' '.$value->getProducto()->getMarcaProducto().' '.LubriUIUtils::formatMontoToView($value->getPrecioUnitario()).'-';
									}
									$productos.=')';
										$html.='<tr><td>'.$combo->getNombre().' '.$productos.'</td><td style="text-align: right;">'.LubriUIUtils::formatMontoToView( $combo->getPrecio() ).'</td></tr>';
