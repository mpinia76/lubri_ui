...
class BalanceDia extends RastyComponent{
	...
	protected function parseXTemplate(XTemplate $xtpl){
		...
		$xtpl->assign("fecha",  LubriUIUtils::formatDateToView( $criteria->getFecha(), "D d M Y") );


		/*$gastos = UIServiceFactory::getUIGastoService()->getTotalesCuenta( null,$this->getFecha() );
		$xtpl->assign("gastos",  LubriUIUtils::formatMontoToView($gastos) );

		$pagos = UIServiceFactory::getUIVentaService()->getTotalesPagosCtaCte( LubriUtils::getCuentaUnica(),$this->getFecha() );
		$xtpl->assign("pagos",  LubriUIUtils::formatMontoToView($pagos) );

		$ventas = UIServiceFactory::getUIVentaService()->getTotalesCuenta( null,$this->getFecha() );
		$xtpl->assign("ventas",  LubriUIUtils::formatMontoToView($ventas) );*/
		...
		$xtpl->assign("ganancias",  LubriUIUtils::formatMontoToView($saldos['ganancias']) );
		$xtpl->assign("ventas",  LubriUIUtils::formatMontoToView($saldos['ventas']) );
        $xtpl->assign("gastos",  LubriUIUtils::formatMontoToView((-1)*$gastoSaldo)  );
		if ($saldos['productos']) {
			$productos='';
