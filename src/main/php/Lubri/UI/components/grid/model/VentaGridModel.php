...
use Lubri\UI\components\grid\formats\GridEstadoVentaFormat;
...
//		//TODO esto setearlo en el .rasty
	...
	protected function initModel() {
        ...
        $column = GridModelBuilder::buildColumn( "modeloPatente", "venta.auto", 20, EntityGrid::TEXT_ALIGN_LEFT );

        $this->addColumn( $column );

        $column = GridModelBuilder::buildColumn( "km", "venta.km", 20, EntityGrid::TEXT_ALIGN_RIGHT);
	...
	public function getRowStyleClass($item){
		...
		return LubriUIUtils::getEstadoVentaCss($item->getEstado());
	...
	public function getHeaderContent(){
		...
		return 'Bruto: <strong>'.LubriUIUtils::formatMontoToView($service->getTotales($filter->getCriteria())).'</strong> Neto: <strong>'.LubriUIUtils::formatMontoToView($service->getGanancias($filter->getCriteria())).'</strong>';*/
	}
