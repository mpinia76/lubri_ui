...
namespace Lubri\UI\components\boxes\venta;
...
class VentaBox extends RastyComponent{
	...
	protected function parseLabels(XTemplate $xtpl){
        ...
        $xtpl->assign("lbl_auto",  $this->localize( "venta.auto" ) );
	...
	protected function parseXTemplate(XTemplate $xtpl){
        ...
        $xtpl->assign( "auto", $this->getVenta()->getModeloPatente() );

		$xtpl->assign( "monto", LubriUIUtils::formatMontoToView( $this->getVenta()->getMonto() ) );
		$xtpl->assign( "montoPagado", LubriUIUtils::formatMontoToView( $this->getVenta()->getMontoPagado() ) );

		$xtpl->assign( "montoDebe", LubriUIUtils::formatMontoToView( $this->getVenta()->getMontoDebe() ) );
		$xtpl->assign( "montoTotal", $this->getVenta()->getMontoDebe() );


		$xtpl->assign( "observaciones", $this->getVenta()->getObservaciones() );
		$xtpl->assign( "fecha", LubriUIUtils::formatDateTimeToView($this->getVenta()->getFecha()) );
		...
		foreach ($venta->getDevoluciones() as $devolucion) {
			...
			$xtpl->assign( "precio", LubriUIUtils::formatMontoToView( $devolucion->getPrecioUnitario() ) );
			$xtpl->assign( "subtotal", LubriUIUtils::formatMontoToView( $devolucion->getSubtotal() ) );
			$xtpl->parse( "main.devolucion" );*/

			$htmlItem .='<tr><td>'.$devolucion->getProducto().'</td><td class="align-r">'.$devolucion->getCantidad().'</td><td class="align-r">'.LubriUIUtils::formatMontoToView( $devolucion->getPrecioUnitario() ).'</td><td class="align-r">'.LubriUIUtils::formatMontoToView( $devolucion->getSubtotal() ).'</td></tr>';
		...
		if ($cantidadTotal) {
			$html.='<div class="box_details">
					<table>
						...
						<tfoot>
							<tr><td>'.$this->localize( "venta.detalle.subtotal" ).'</td><td class="align-r">'.$cantidadTotal.'</td><td></td><td class="align-r">'.LubriUIUtils::formatMontoToView( $venta->getMontoDevolucion() ).'</td></tr>
						</tfoot>
					</table>
				</div>';
		}
